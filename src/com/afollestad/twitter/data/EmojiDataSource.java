package com.afollestad.twitter.data;
...
public class EmojiDataSource {
    ...
    public EmojiRecent createRecent(String text, String icon) {
        try {
            ...
            Cursor cursor = database.query(EmojiSQLiteHelper.TABLE_RECENTS,
                    ...
                    null, null, null);
            cursor.moveToFirst();
            EmojiRecent newRecent = cursorToRecent(cursor);
            cursor.close();
            return newRecent;
        } catch (Exception e) {
            return null;
    ...
    public List<EmojiRecent> getAllRecents() {
        List<EmojiRecent> recents = new ArrayList<EmojiRecent>();
        ...
        while (!cursor.isAfterLast()) {
            if (cursor.getPosition() >= NUM_RECENTS_TO_SAVE) {
                ...
                EmojiRecent recent = cursorToRecent(cursor);
                recents.add(recent);
            }
    ...
    private EmojiRecent cursorToRecent(Cursor cursor) {
        EmojiRecent recent = new EmojiRecent();
