...
public class ProfileFollowingViewer extends BoidListFragment<User> {
    ...
    @Override
    protected void onPostLoad(List<User> results, boolean paginated, boolean loadComplete) {
        super.onPostLoad(results, paginated, loadComplete);
        if (results == null || results.size() == 0 || getAdapter().getCount() == mUser.getFriendsCount()) {
